"""Long-running subprocess relay for DS Team Runtime Contract Focused Review.
Launches Claude Code with agent team (2-3 reviewers) + MCP.
Timeout: 900s. Max turns: 40. Focused contract review.
"""
import subprocess, json, sys, threading, time, os
from pathlib import Path
from datetime import datetime

TASK_ID = "pm-runtime-governance/ds-runtime-contract-review"
...
def write_progress():
    elapsed = int(time.time() - start_time)
    ds_report = (output_dir / "ds/ds_runtime_contract_review.md").exists()
...
prompt = (
    "下面是完整任务书（通过stdin传入）。严格按任务书执行：PM Runtime Communication Substrate Runtime Contract v0.1 聚焦审查。"
    "必须开启Agent Team（≥2 reviewer：P0-Repair-Verification / Task-Card-Readiness）。"
    ...
    "receipt_yaml: {task_id, review_type, team_mode_used, mcp_used, acceptance_verdict, "
    "p0_repair_status: {P0-1_yaml_directory, P0-2_pre_action_gate, P0-3_registry_status, "
    "P0-4_partial_output, P0-5_authority_vacuum, P0-6_anti_tamper, P0-7_evidence_fidelity}, "
    "findings: {P0: [], P1: [], P2: [], P3: []}, "
    "task_card_readiness_assessment: {sufficient, gaps: []}, "
    "iteration_doc_format_recommendation: {suitable, allowed_files: [], forbidden_files: [], required_checks: [], receipt_fields: [], notes}, "
...
try:
    ...
    if result.returncode == 0:
        ...
        # Path 1: permission_denial payload (sort by length)
        denial_candidates = []
        ...
        if len(denial_candidates) >= 1:
            (output_dir / "ds/ds_runtime_contract_review.md").write_text(denial_candidates[0], encoding="utf-8")
        ...
        if not report_written and inner:
            try:
                ...
                if "report_markdown" in inner_json:
                    (output_dir / "ds/ds_runtime_contract_review.md").write_text(inner_json["report_markdown"], encoding="utf-8")
                    report_written = True
