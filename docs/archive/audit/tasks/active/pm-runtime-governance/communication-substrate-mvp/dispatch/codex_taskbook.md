...
> status: owner_approved / ready_for_codex  
...
任务书已通过 Owner final approval 和 DS re-review（`pass_with_known_issues, codex_readiness: ready`）。
...
✅ DS Team quick re-review → pass_with_known_issues
✅ Owner final approval
✅ Hermes copies approved taskbook to docs/iterations/
→ Codex executes（当前）
...
v0.1 MVP supports single-task execution only. Concurrent task execution, worker pool, daemon, queue, and lock manager are out of scope.
...
`tools/pm_runtime/templates/task_config.yaml` is the reusable template. `dispatch/task_config.yaml` is the per-task instantiated config.
