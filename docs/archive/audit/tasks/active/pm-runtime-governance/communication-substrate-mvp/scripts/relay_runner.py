"""Long-running subprocess relay for DS Team Codex Taskbook Pre-Implementation Review.
Launches Claude Code with agent team (2-3 reviewers) + MCP.
Timeout: 900s. Max turns: 40.
"""
...
def write_progress():
    elapsed = int(time.time() - start_time)
    ds_report = (output_dir / "ds/ds_codex_taskbook_review.md").exists()
...
prompt = (
    "下面是完整任务书（通过stdin传入）。严格按任务书执行：PM Runtime Communication Substrate MVP Codex Taskbook v0.1 预实现审查。"
    "必须开启Agent Team（≥2 reviewer：Scope-Boundary-Verification / Executability-Assessment / Contract-Alignment）。"
    ...
    "codex_readiness: {ready, gaps: []}, "
    "scope_boundary_assessment: {allowed_files_complete, forbidden_files_complete, missing_from_allowed: [], missing_from_forbidden: []}, "
    "runtime_artifact_coverage: {contract_artifacts, taskbook_artifacts, gaps: []}, "
...
try:
    ...
    if result.returncode == 0:
        ...
        if len(denial_candidates) >= 1:
            (output_dir / "ds/ds_codex_taskbook_review.md").write_text(denial_candidates[0], encoding="utf-8")
        ...
        if not report_written and inner:
            try:
                ...
                if "report_markdown" in inner_json:
                    (output_dir / "ds/ds_codex_taskbook_review.md").write_text(inner_json["report_markdown"], encoding="utf-8")
                    report_written = True
