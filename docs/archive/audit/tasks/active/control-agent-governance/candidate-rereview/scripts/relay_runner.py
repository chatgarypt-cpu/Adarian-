"""Long-running subprocess relay for DS Team Candidate Re-review.
Launches Claude Code with agent team (4 reviewers) + MCP.
Timeout: 900s. Max turns: 40. Lighter review than full audit.
"""
import subprocess, json, sys, threading, time, os
from pathlib import Path
from datetime import datetime

TASK_ID = "control-agent-governance/candidate-rereview"
...
def write_progress():
    elapsed = int(time.time() - start_time)
    ds_report = (output_dir / "ds/ds_candidate_rereview.md").exists()
...
prompt = (
    "下面是完整任务书（通过stdin传入）。严格按任务书执行：Control Agent Governance Assets 候选稿复审。"
    "必须开启Agent Team（≥4 reviewer：Format / System Prompt Slimming / Hermes-first / Layering-Authority）。"
    "必须使用MCP filesystem工具读取 3 个候选稿文件。只读复审，不修改任何文件。"
    "将所有审查结果以完整JSON格式返回在你的result中。"
    "JSON结构：{report_markdown: 完整的中文Markdown复审报告, "
    "receipt_yaml: {task_id, review_id, team_mode_used, mcp_used, "
    "reviewed_files: [], p0_status, p1_status, system_prompt_slimming_verdict, "
    "hermes_first_alignment, file_first_alignment, layering_alignment, "
    "remaining_findings: [], blockers: [], process_issues: [], "
    "acceptance_verdict, report_path}}"
)
...
try:
    ...
    if result.returncode == 0:
        ...
        for pd in claude_result.get("permission_denials", []):
            if pd["tool_name"] == "mcp__filesystem__write_file":
                ...
                if "report_markdown" in content or "ds_candidate_rereview" in content:
                    (output_dir / "ds/ds_candidate_rereview.md").write_text(content, encoding="utf-8")
        ...
        if not report_written:
            try:
                ...
                if "report_markdown" in inner_json:
                    (output_dir / "ds/ds_candidate_rereview.md").write_text(inner_json["report_markdown"], encoding="utf-8")
    ...
    RESULT.write_text(json.dumps({"error": "timeout"}, ensure_ascii=False), encoding="utf-8")
except Exception as e:
