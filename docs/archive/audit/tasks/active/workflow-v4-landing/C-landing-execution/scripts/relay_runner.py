"""Long-running subprocess relay for Landing Execution Plan Review.
...
def write_progress():
    ...
    report = (output_dir / "summary/workflow_landing_execution_plan_review_2026-05-19.md").exists()
...
prompt = (
    "下面是完整任务书（通过stdin传入）。严格按任务书执行：Workflow Core v4.0 Landing Execution Plan Review。"
    "必须使用MCP filesystem工具读取项目文件。只读审查，不修改任何文件。"
    "请将所有审查结果以完整JSON格式返回在你的result中。"
    "JSON结构：{report_markdown: 完整的中文Markdown审查报告文本, "
    "result_yaml: {task_id, verdict, first_landing_batch: {executor, purpose, "
    "allowed_files: [], forbidden_files: [], required_commands: [], "
    "required_static_checks: [], required_receipts: [], post_landing_review, "
    "commit_mode, stop_conditions: []}, blockers: [], recommended_next_action, report_path}}"
)
    ...
    if result.returncode == 0:
        ...
        try:
            ...
            if "report_markdown" in inner_json:
                (output_dir / "summary/workflow_landing_execution_plan_review_2026-05-19.md").write_text(inner_json["report_markdown"], encoding="utf-8")
