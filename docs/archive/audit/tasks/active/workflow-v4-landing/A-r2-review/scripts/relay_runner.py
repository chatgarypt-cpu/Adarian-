"""Long-running subprocess relay for DS Team R2 Structural Review.
Launches Claude Code with agent team + MCP, writes heartbeat/progress in real time.
Timeout: 1500s. Max turns: 60. Team mode REQUIRED (5 reviewers).
"""
...
def write_progress():
    ...
    ds_report = (output_dir / "ds/ds_review_report_workflow_core_v4_r2_2026-05-19.md").exists()
...
prompt = (
    "下面是完整任务书（通过stdin传入）。严格按任务书执行：Workflow Core v4.0 R2 结构审查。"
    "必须开启Agent Team（≥5 reviewer subagent：Structure Reviewer / Workflow Semantics Reviewer / "
    "Path & Artifact Governance Reviewer / HOLD & Patch Lane Reviewer / Landing Risk Reviewer）。"
    "必须使用MCP filesystem工具。只读审计，不修改任何文件。"
    ...
    "blockers, known_issues, process_issues, can_enter_codex_landing, "
    "report_path, receipt_path, reviewer_agents, recommended_next_action}}"
)
    ...
    if result.returncode == 0:
        ...
        try:
            ...
            if "report_markdown" in inner_json:
                (output_dir / "ds/ds_review_report_workflow_core_v4_r2_2026-05-19.md").write_text(inner_json["report_markdown"], encoding="utf-8")
