"""Long-running subprocess relay for DS Team YAML Machine-Readability Review.
...
TASK_ID = "workflow-v4-landing/yaml-review"
...
def write_progress():
    elapsed = int(time.time() - start_time)
    ds_report = (output_dir / "ds/ds_yaml_machine_review.md").exists()
...
prompt = (
    "下面是完整任务书（通过stdin传入）。严格按任务书执行：Workflow Compact YAML Machine-Readability Review。"
    "必须开启Agent Team（≥4 reviewer：YAML Schema / Machine Readability / Auto-Generation / Risk）。"
    "必须使用MCP filesystem工具读取 YAML 文件。只读审查，不修改任何文件。"
    "将所有审查结果以完整JSON格式返回在你的result中。"
    "JSON结构：{report_markdown: 完整中文审查报告, "
    "receipt_yaml: {task_id, review_id, team_mode_used, mcp_used, "
    "yaml_parse_status, machine_readability_verdict, schema_recommendation, "
    "blockers: [], major_findings: [], minor_findings: [], "
    "field_verdicts: {}, recommended_next_action, report_path}}"
)
    ...
    if result.returncode == 0:
        ...
        for pd in claude_result.get("permission_denials", []):
            if pd["tool_name"] == "mcp__filesystem__write_file":
                ...
                if "report_markdown" in content or "yaml_machine_review" in content:
                    (output_dir / "ds/ds_yaml_machine_review.md").write_text(content, encoding="utf-8")
        ...
        if not report_written:
            try:
                ...
                if "report_markdown" in inner_json:
                    (output_dir / "ds/ds_yaml_machine_review.md").write_text(inner_json["report_markdown"], encoding="utf-8")
