    ...
    <PageState :state="state" message="报告生成失败">
      <div class="hero-grid">
        <Panel title="报告生成" note="选择推演结果">
          <div class="mock-note">当前报告生成不调用 Phase 4；真实报告入口在 v1.5.0b 接入。</div>
          ...
          <div class="actions"><button class="primary" type="button" @click="generate">生成报告草稿</button><button class="ghost" type="button">预览报告结构</button></div>
        </Panel>
...
async function generate() {
  state.value = 'loading';
  files.value = await api.getReports();
